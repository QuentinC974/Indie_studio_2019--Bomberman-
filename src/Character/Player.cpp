...
Player::Player(Core *core, Map *map, float x, float z, float r, irr::video::ITexture *tex)
{
    ...
    this->_anms = this->_smgr->addAnimatedMeshSceneNode(this->_smgr->getMesh("../assets/media/m_s.b3d"), 0, -1, irr::core::vector3df(x, this->_core->getMapScale() * 2 * 2, z), irr::core::vector3df(0, r, 0));
    if (!this->_anms)
...
void Player::killPlayer(int id)
{
    ...
    std::cout << "Steve " << id << " f*cking exploded." << std::endl;
